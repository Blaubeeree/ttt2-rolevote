...
function RoleVote:End()
	...
	hook.Add("TTT2RoleNotSelectable", "TTTRolevoteDisableRoles", function(role)
		if GetConVar("ttt_rolevote_voteban"):GetBool() then
			...
			not table.HasValue(always_active, role.abbr) or
			nil
		end
	end)
end
...
hook.Add("Initialize", "TTTRolevoteInitialize", function()
	...
	hook.Add("TTTPrepareRound", "TTTRolevotePrepareRound", function()
		if not firstPrep then
			timer.Create(
				"RoleVote_PrepTimer",
				GetConVar("ttt_preptime_seconds"):GetInt(),
				1,
				PrepTimerFinished
			)
		else
			timer.Create(
				"RoleVote_PrepTimer",
				GetConVar("ttt_firstpreptime"):GetInt(),
				1,
				PrepTimerFinished
			)
		end
	...
	hook.Add("PlayerSpawn", "TTTRolevotePlayerSpawn", function()
		if GetConVar("ttt_rolevote_min_players"):GetInt() > #player.GetAll() or
			RoleVote.started
		then return end
		RoleVote:Start(-1)
	end)
...
concommand.Add("printRoles", function(ply)
	local function addRoles(active, tbl)
		...
		for _, role in pairs(GetRoles()) do
			if active and
				role:IsSelectable(false) or
				not active and
				not role:IsSelectable(false) and
				not role.notSelectable
			then
				i = i + 1
